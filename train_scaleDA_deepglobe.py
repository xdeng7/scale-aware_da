...
from dataset.ISPRS_dataset_multiscale_v2 import ISPRS_dataset_multi
from dataset.DeepGlobe_crop_dataset import DeepGlobe_dataset

import argparse

from modeling.deeplab import DeepLab
from modeling.scalenet_deeplab import DeepLabCA
...
def main():
    ...
    target_set = ISPRS_dataset_multi(5,train_ids,DATA_FOLDER1, LABEL_FOLDER1,DATA_FOLDER2,LABEL_FOLDER2,cache=args.cache)
    target_loader = torch.utils.data.DataLoader(target_set,batch_size=args.batch_size)

    # val_set = ISPRS_dataset(val_ids, DATA_FOLDER1, LABEL_FOLDER1,cache=args.cache)
    # val_loader = torch.utils.data.DataLoader(val_set,batch_size=args.batch_size)

    LABELS = ["roads", "buildings", "low veg.", "trees", "cars", "clutter","unknown"] # Label names
